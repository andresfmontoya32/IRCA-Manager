...
class WorkflowController:
    ...
    def prepare_download_package(self, output_directory: str = None) -> Tuple[bool, str, str]:
        """Prepara paquete de descarga SOLO con los reportes Word generados"""
        try:
            import shutil
            from pathlib import Path
            from datetime import datetime
            
            # Si no se proporciona directorio, usar uno por defecto
            import os
            desktop = Path(os.path.expanduser("~")) / "Desktop"
            
            if not output_directory:
                output_directory = str(desktop / "Reportes_IRCA")
            ...
            if not output_dir.exists():
                try:
                    ...
                    return False, f"❌ No se pudo crear carpeta destino: {str(e)}", ""
            
            # Crear carpeta con nombre del mes y timestamp
            session_info = settings.get_session_info()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            
            if session_info['has_month_selected']:
                folder_name = f"Reportes_IRCA_{session_info['selected_month']}_{session_info['selected_year']}_{timestamp}"
            else:
                folder_name = f"Reportes_IRCA_{timestamp}"
            
            month_folder = output_dir / folder_name
            if month_folder.exists():
                shutil.rmtree(month_folder)
            month_folder.mkdir()
            
            # Buscar y copiar SOLO los reportes Word generados
            ciudades = settings.get_carpetas_datos()
            reportes_copiados = 0
            ...
            for ciudad in ciudades:
                ...
                if ciudad_path.exists():
                    ...
                    if reporte_file.exists() and reporte_file.stat().st_size > 1024:  # Mínimo 1KB
                        ...
                        if session_info['has_month_selected']:
                            nombre_destino = f"Reporte_IRCA_{ciudad}_{session_info['selected_month']}_{session_info['selected_year']}.docx"
                        else:
                            nombre_destino = f"Reporte_IRCA_{ciudad}.docx"
                        
                        destino = month_folder / nombre_destino
                        shutil.copy2(reporte_file, destino)
                        reportes_copiados += 1
                        reportes_encontrados.append(ciudad)
                        print(f"✅ Copiado: {reporte_file} → {destino}")
                    else:
                        print(f"⚠️ Reporte no encontrado o vacío para {ciudad}")
            
            if reportes_copiados == 0:
                # Verificar si existen reportes en general
                all_reports = []
                for ciudad in ciudades:
                    ciudad_path = settings.DATOS_DIR / ciudad
                    reporte_file = ciudad_path / f"reporte_{ciudad}.docx"
                    if reporte_file.exists():
                        all_reports.append(f"{ciudad}: {reporte_file.stat().st_size} bytes")
                
                error_msg = "❌ No se encontraron reportes Word válidos"
                if all_reports:
                    error_msg += f"\n🔍 Reportes encontrados pero muy pequeños:\n" + "\n".join(all_reports)
                else:
                    error_msg += "\n💡 Ejecute el flujo completo para generar reportes"
                
                return False, error_msg, ""
            
            # Crear archivo de resumen
            resumen_file = month_folder / "RESUMEN_DESCARGA.txt"
            with open(resumen_file, 'w', encoding='utf-8') as f:
                f.write(f"DESCARGA DE REPORTES IRCA\n")
                f.write(f"{'='*50}\n\n")
                f.write(f"Período: {session_info.get('month_display', 'No especificado')}\n")
                f.write(f"Fecha descarga: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n")
                f.write(f"Total reportes: {reportes_copiados}\n\n")
                f.write(f"Ciudades incluidas:\n")
                for ciudad in reportes_encontrados:
                    f.write(f"  ✅ {ciudad}\n")
                
                if output_directory != str(desktop / "Reportes_IRCA"):
                    f.write(f"\nCarpeta personalizada: {output_directory}\n")
            
            return True, f"✅ {reportes_copiados} reportes Word descargados exitosamente", str(month_folder)
            
        except Exception as e:
            import traceback
            error_detail = traceback.format_exc()
            print(f"Error en prepare_download_package: {error_detail}")
            return False, f"❌ Error preparando descarga: {str(e)}", ""
